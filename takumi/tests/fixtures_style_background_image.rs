...
#[test]
fn test_background_image_noise_v1_with_creative_gradient() {
  let images = BackgroundImagesValue::Css(
    "radial-gradient(circle at 25% 25%, rgba(255, 0, 128, 0.6), transparent 50%), radial-gradient(circle at 75% 75%, rgba(0, 128, 255, 0.6), transparent 50%), linear-gradient(135deg, rgba(138, 43, 226, 0.4), rgba(30, 144, 255, 0.4), rgba(255, 20, 147, 0.4)), noise-v1(0.01)".to_string(),
  );
  ...
  run_style_width_test(
    container.into(),
    "tests/fixtures/style_background_image_noise_v1_creative_blend.png",
  );
}
