...
fn test_ttf_font_loading() {
  assert!(CONTEXT.font_context.load_font(TTF_FONT.to_vec()).is_ok());
}

#[test]
fn test_woff_font_loading() {
  assert!(CONTEXT.font_context.load_font(WOFF_FONT.to_vec()).is_ok());
}

#[test]
fn test_woff2_font_loading() {
  assert!(CONTEXT.font_context.load_font(WOFF2_FONT.to_vec()).is_ok());
}
...
fn test_invalid_format_detection() {
  ...
  let result = CONTEXT.font_context.load_font(invalid_data);
...
fn test_empty_data() {
  ...
  let empty_data = vec![];
  let result = CONTEXT.font_context.load_font(empty_data);
  assert!(matches!(result, Err(FontError::UnsupportedFormat)));
}
...
fn test_too_short_data() {
  ...
  let result = CONTEXT.font_context.load_font(short_data);
  assert!(matches!(result, Err(FontError::UnsupportedFormat)));
}
