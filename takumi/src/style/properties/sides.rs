  ...
  fn try_from(value: SidesValue<T>) -> Result<Self, Self::Error> {
    match value {
      ...
      SidesValue::Css(string) => {
        ...
        let sides = match (second, third, fourth) {
          (None, _, _) => Sides([first; 4]),
          (Some(h), None, _) => Sides([first, h, first, h]),
          (Some(r), Some(b), None) => Sides([first, r, b, r]),
          (Some(r), Some(b), Some(l)) => Sides([first, r, b, l]),
        };
...
mod tests {
  use super::*;
  use crate::style::properties::length_unit::LengthUnit;
  use serde_json;
