...
/// # Example
/// ```rust
/// use takumi::{impl_node_enum, ContainerNode, ImageNode, TextNode};
///
/// #[derive(Debug, Clone)]
/// enum NodeKind {
///   Image(ImageNode),
...
macro_rules! impl_node_enum {
  ($name:ident, $($variant:ident => $variant_type:ty),*) => {
    impl $crate::layout::Node<$name> for $name {
      ...
      fn get_style(&self) -> &$crate::style::Style {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::get_style(inner), )*
        }
      }

      fn get_style_mut(&mut self) -> &mut $crate::style::Style {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::get_style_mut(inner), )*
        }
      }

      fn inherit_style(&mut self, parent: &$crate::style::Style) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::inherit_style(inner, parent), )*
        }
      }

      fn before_layout(&mut self) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::before_layout(inner), )*
        }
      }

      fn inherit_style_for_children(&mut self) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::inherit_style_for_children(inner), )*
        }
      }

      fn should_hydrate(&self) -> bool {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::should_hydrate(inner), )*
        }
      }
      ...
      fn measure(
        &self,
        context: &$crate::core::RenderContext,
        ...
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::measure(inner, context, available_space, known_dimensions), )*
        }
      }

      fn draw_on_canvas(&self, context: &$crate::core::RenderContext, canvas: &mut $crate::rendering::FastBlendImage, layout: $crate::taffy::Layout) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::draw_on_canvas(inner, context, canvas, layout), )*
        }
      }
      ...
      fn draw_content(&self, context: &$crate::core::RenderContext, canvas: &mut $crate::rendering::FastBlendImage, layout: $crate::taffy::Layout) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::draw_content(inner, context, canvas, layout), )*
        }
      }

      fn draw_border(&self, context: &$crate::core::RenderContext, canvas: &mut $crate::rendering::FastBlendImage, layout: $crate::taffy::Layout) {
        match self {
          $( $name::$variant(inner) => <_ as $crate::layout::Node<$name>>::draw_border(inner, context, canvas, layout), )*
        }
      }
